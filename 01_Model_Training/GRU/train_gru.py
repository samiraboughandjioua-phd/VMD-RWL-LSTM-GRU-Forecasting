# ==========================================================
# Multi-Horizon Hourly Water-Level Forecasting - GRU
# Single-run implementation for hourly river water-level forecasting.
#
# The script trains one GRU configuration for three stations and eight
...
# settings, and random seed as the LSTM implementation so that the recurrent
...
parser = argparse.ArgumentParser(description="Train the GRU model for hourly RWL forecasting.")
...
parser.add_argument("--output-dir", default=os.path.join(PROJECT_ROOT, "outputs", "Results_GRU_80_20_SINGLE_RUN_FINAL"),
...
# is used by the GRU and LSTM scripts.
FORECAST_CONFIG = {
...
# ----------------------------------------------------------
# 9. GRU architecture: 64 -> 32 (IDENTICAL to GRU script)
# ----------------------------------------------------------
def build_model(input_shape):
    model = Sequential([
        Input(shape=input_shape),
        GRU(UNITS_1, activation="tanh", return_sequences=True),
        Dropout(DROPOUT),
        GRU(UNITS_2, activation="tanh", return_sequences=False),
        Dropout(DROPOUT),
        Dense(1),
    ])
    model.compile(
...
for station_name, file_path in STATIONS.items():
    ...
    for horizon, lag in FORECAST_CONFIG.items():
        ...
        print("\n" + "-" * 72)
        print(f"GRU | {station_name} | Horizon={horizon} h | Lag={lag}")
        print("-" * 72)
        ...
        summary_rows.append({
            ...
            "GRU_R2": lstm_metrics["R2"], "GRU_NSE": lstm_metrics["NSE"],
            "GRU_KGE": lstm_metrics["KGE"], "GRU_RMSE": lstm_metrics["RMSE"],
            "GRU_MAE": lstm_metrics["MAE"],
            ...
            "DM_GRU_vs_Naive": dm_stat, "p_GRU_vs_Naive": p_val,
            "Signif_GRU_vs_Naive": significance_label(p_val),
            "GRU_Flood_RMSE": lstm_flood["Flood_RMSE"],
            "GRU_Flood_MAE": lstm_flood["Flood_MAE"],
            "GRU_Flood_Bias": lstm_flood["Flood_Bias"],
            "GRU_Peak_Error": lstm_flood["Peak_Error"],
            ...
            "Naive_Peak_Error": naive_flood["Peak_Error"],
                        "GRU_Resid_Bias": resid["Resid_Bias"], "GRU_Resid_SD": resid["Resid_SD"],
            "GRU_LjungBox_p": resid["LjungBox_p"], "GRU_Shapiro_p": resid["Shapiro_p"],
        })

        # Prediction CSV -- EXACT filename/columns expected downstream
        pred_df = pd.DataFrame({
            "Date": timestamps, "Actual": y_true,
            "GRU_Predicted": y_fore, "Naive_Predicted": y_naive,
        })
        pred_file = os.path.join(
            station_dir, f"GRU_{station_name}_horizon_{horizon}h_predictions.csv"
        )
        pred_df.to_csv(pred_file, index=False)

        print(f"Saved: {pred_file}")
        print(
            f"[GRU]  R2={lstm_metrics['R2']:.4f} | NSE={lstm_metrics['NSE']:.4f} | "
        ...
        print(f"DM GRU vs Naive: stat={dm_stat:.4f} | p={p_val:.4f}")
        ...
        summary_df.to_csv(
            os.path.join(RESULTS_DIR, "GRU_results_summary_SINGLE_RUN.csv"), index=False
        )

print(f"\nSaved summary: "
      f"{os.path.join(RESULTS_DIR, 'GRU_results_summary_SINGLE_RUN.csv')}")
print(f"\n=== GRU (SINGLE-RUN COMPLETE) FINISHED SUCCESSFULLY ===")
